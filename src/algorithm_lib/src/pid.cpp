#include "algorithm_lib/Alg_PID.hpp"
 ...
 */
PID_c::PID_c(PidInitConfig_t pid_config):
...
out_filter(pid_config.out_filter_num)
{
    ...
    else this->ActualValueSource_ = NULL;
...
//无参构造函数
PID_c::PID_c(): out_filter(0), d_filter(0)
{
...
{
    if (std::abs(this->measure_.error) <= this->measure_.errorabsmin) //最小值
    {
        return 1.0f;
    }
    else if (std::abs(this->measure_.error) > this->measure_.errorabsmax) //最大值
    {
        return 0.0f;
    }
    else
    {
        return (this->measure_.errorabsmax - abs(this->measure_.error)) / (this->measure_.errorabsmax - this->measure_.errorabsmin);
    }
}
...
//修改步长
void PID_c::ResetStepIn(float StepIn)
{
...
float* PID_c::GetActValSource(void)
{
    if(this->ActualValueSource_ != NULL)
...
//串级PID构造函数
CascadePID_c::CascadePID_c(PidInitConfig_t outer_pid_config, PidInitConfig_t inter_pid_config, float output_limit):
outer_pid(outer_pid_config),inter_pid(inter_pid_config),Output_Limit(output_limit)
{
...
float CascadePID_c::Calc(const float *SetPosVal)
{
    Target = SetPosVal;
    //计算外环
    outer_pid_output = (Outer_ActualVal==nullptr) ?
                       outer_pid.Calc(*SetPosVal, 0) : outer_pid.Calc(*SetPosVal, *Outer_ActualVal);
    //加入速度前馈
    if(SpeedFeedWard != nullptr)
        outer_pid_output += *SpeedFeedWard;
    //计算内环
    inter_pid_output = (Inter_ActualVal==nullptr) ?
                       inter_pid.Calc(outer_pid_output, 0) : inter_pid.Calc(outer_pid_output, *Inter_ActualVal);
...
float CascadePID_c::Calc(const float *SetPosVal, const float *outer_ActValSource, const float *inter_ActValSource)
{
    Target = SetPosVal;
    //计算外环
    outer_pid_output = (outer_ActValSource==nullptr) ?
                       outer_pid.Calc(*SetPosVal, 0) : outer_pid.Calc(*SetPosVal, *outer_ActValSource);
    //加入速度前馈
    if(SpeedFeedWard != nullptr)
        outer_pid_output += *SpeedFeedWard;
    //计算内环
    inter_pid_output = (inter_ActValSource==nullptr) ?
                       inter_pid.Calc(outer_pid_output, 0) : inter_pid.Calc(outer_pid_output, *inter_ActValSource);
    user_value_limit(inter_pid_output,-Output_Limit,Output_Limit);
    return inter_pid_output;
}
